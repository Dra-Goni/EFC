...
@include('layout.navbar')
    ...
    <div class="form-row">
        <div class="form-group col-12 text-center">
            <h5>Todas las Inasistencias No Respondidas </h5>
        </div>
    </div>
        ...
        <table class="table table-hover">
            ...
            <tbody>
                    ...
                    <tr>
                        ...
                        <td >
                            <form action="{{url('/seguimiento/no_cor')}}" method="POST" >
                                @csrf
                                <input type="hidden" value="{{$item->INA_CODIGO}}" name="codigo"> 
                                <input type="submit" value="LLamada No Respondida" class="btn btn-danger"> 
                            </form>
                        </td> 
                        <td> <form action="{{url('/seguimiento/respuesta_cor')}}" method="POST" >
                            @csrf
                            <input type="hidden" value="{{$item->INA_CODIGO}}" name="codigo"> 
                            <input type="submit" value="LLamada Respondida" class="btn btn-success"> 
                            </form>
                        </td>
                    </tr>
                    @endforeach
